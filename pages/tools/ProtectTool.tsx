  ...
  const handleProtect = async () => {
    ...
    try {
      ...
      setState({
        ...
        error: null,
        resultUrl: url,
        resultName: `protected_${file.name}`
      });
      toast.success("PDF Protected successfully!");
    } catch (err: any) {
      console.error(err);
      setState(prev => ({ ...prev, isProcessing: false, error: "Protection failed." }));
      ...
      {!file ? (
        ...
        <div className="max-w-xl mx-auto space-y-8 animate-fade-in">
          <div className="bg-white p-6 rounded-xl border border-gray-200">
             <label className="block text-sm font-medium text-gray-700 mb-2">Set a Password</label>
          ...
          {!state.resultUrl ? (
            <div className="flex justify-center">
              ...
              >
                {state.isProcessing ? (
                  <>
                    <Loader2 className="animate-spin w-6 h-6" /> Encrypting...
                  </>
                ) : (
                  <>
                    Protect PDF <ArrowRight className="w-6 h-6" />
                  </>
                )}
              </button>
            </div>
